...
namespace Yammy
{
	...
	class Decoder
	{
		...
		public Decoder(string filePath, string ownerID, string partnerID)
		{
			m_bSucceeded = false;
			m_strFilePath = filePath;
            m_bSucceeded = true;
            m_strEncryptID = ownerID;
            m_strLocalID = m_strEncryptID;
            m_strRemoteID = partnerID;
		...
		public string Decode(bool raw, bool preview, string highlight)
		{
			...
			while (true)
			{
                ...
                //Format time
                //strTime = string.Format("{0:yyyy-MM-dd HH:mm:ss}", MakeDTFromCTime(timeStamp));
                strTime = MakeDTFromCTime(timeStamp).ToString("G");
				if (!raw)
				{
